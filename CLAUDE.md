...
- **The origin allowlist covers Cloudflare's IPv4 *and* IPv6 ranges,** and the SSL/TLS mode is **Full (Strict)** — TLS the whole way, with Cloudflare validating the origin's certificate rather than accepting a self-signed one. Confirmed 2026-08-18. If that origin cert ever expires the site goes down at the edge with a 526, which reads as an outage but is a certificate problem. The zone publishes AAAA records, so Cloudflare can reach the origin over v6; an allowlist that drifts to IPv4-only breaks the site in a way that looks like a dead server. Cloudflare rotates these ranges occasionally — they come from `cloudflare.com/ips`, and a stale list is a plausible cause of a sudden sitewide outage with a healthy container.
